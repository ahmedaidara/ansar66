...
const events = [{ name: 'Conférence Annuelle', description: 'Conférence 2025', image: '', datetime: '2025-08-17T15:00:00' }];
const suggestions = [];
const gallery = [];
const messages = [];
const autoMessages = [];
...
function saveToDB(storeName, data) {
  return new Promise((resolve, reject) => {
    const dbRef = ref(db, storeName);
    set(dbRef, Array.isArray(data) ? data : [data])
      .then(() => resolve())
...
async function initDB() {
  const collections = ['members', 'contributions', 'events', 'suggestions', 'gallery', 'messages', 'autoMessages', 'notes', 'internalDocs', 'presidentFiles', 'secretaryFiles', 'library', 'presidentNotes', 'secretaryNotes'];
  for (const store of collections) {
    const data = await loadFromDB(store);
    window[store] = data.length > 0 ? data : window[store] || [];
  }
  updateMembersList();
  ...
  collections.forEach(store => {
    const dbRef = ref(db, store);
    onValue(dbRef, (snapshot) => {
      const data = snapshot.val();
      window[store] = data ? (Array.isArray(data) ? data : [data]) : [];
      if (store === 'members') updateMembersList();
      if (store === 'contributions') updateContributionsAdminList();
      if (store === 'events') updateEventsList();
      if (store === 'gallery') updateGalleryContent();
      if (store === 'messages') updateMessagesList();
      if (store === 'autoMessages') updateAutoMessagesList();
      if (store === 'notes') updateNotesList();
      if (store === 'internalDocs') updateInternalDocsList();
      if (store === 'presidentFiles') updatePresidentFilesList();
      if (store === 'secretaryFiles') updateSecretaryFilesList();
      if (store === 'suggestions') updateSuggestionsList();
      if (store === 'library') updateLibraryContent();
      if (store === 'presidentNotes') updatePresidentNotesList();
      if (store === 'secretaryNotes') updateSecretaryNotesList();
      updateStats();
      updateEventCountdowns();
      updateMessagePopups();
    });
...
document.querySelector('#add-president-note-form').addEventListener('submit', async (e) => {
  e.preventDefault();
  if (!currentUser || currentUser.role !== 'president') return;
  const note = {
...
document.querySelector('#add-secretary-note-form').addEventListener('submit', async (e) => {
  e.preventDefault();
  if (!currentUser || currentUser.role !== 'secretaire') return;
  const note = {
...
function updatePresidentNotesList() {
  ...
  list.innerHTML = presidentNotes
    .filter(n => n.theme.toLowerCase().includes(search) || n.text.toLowerCase().includes(search))
    .map((n, index) => `
      <div class="note-card">
        <p><strong>${n.theme}</strong>: ${n.text}</p>
        <p><small>${new Date(n.timestamp).toLocaleString()}</small></p>
        <button class="cta-button" onclick="deletePresidentNote(${index})">Supprimer</button>
      </div>
    `).join('');
}

async function deletePresidentNote(index) {
  if (!currentUser || currentUser.role !== 'president') return;
  const dbRef = ref(db, `presidentNotes/${index}`);
  await remove(dbRef);
  updatePresidentNotesList();
}
...
function updateSecretaryNotesList() {
  ...
  list.innerHTML = secretaryNotes
    .filter(n => n.theme.toLowerCase().includes(search) || n.text.toLowerCase().includes(search))
    .map((n, index) => `
      <div class="note-card">
        <p><strong>${n.theme}</strong>: ${n.text}</p>
        <p><small>${new Date(n.timestamp).toLocaleString()}</small></p>
        <button class="cta-button" onclick="deleteSecretaryNote(${index})">Supprimer</button>
      </div>
    `).join('');
}

async function deleteSecretaryNote(index) {
  if (!currentUser || currentUser.role !== 'secretaire') return;
  const dbRef = ref(db, `secretaryNotes/${index}`);
  await remove(dbRef);
  updateSecretaryNotesList();
}
...
function updateStats() {
  const totalAmount = members.reduce((sum, m) => {
    if (!m.contributions) return sum;
    return sum + Object.entries(m.contributions).reduce((s, [name, years]) => {
      if (!years) return s;
      return s + Object.values(years).reduce((t, months) => {
        if (!Array.isArray(months)) return t;
        return t + months.filter(p => p).length * (contributions.find(c => c.name === name)?.amount || 0);
      }, 0);
    }, 0);
  }, 0);
  const membersCount = members.length;
  const activeMembers = members.filter(m => m.status === 'actif').length;
  const upToDateMembers = members.filter(m => {
    if (!m.contributions) return false;
  ...
  const ctxTotal = document.getElementById('stats-total-amount');
  if (ctxTotal) {
    new Chart(ctxTotal, {
      type: 'bar',
      data: {
  ...
  const ctxMembers = document.getElementById('stats-members');
  if (ctxMembers) {
  ...
  const ctxStatus = document.getElementById('stats-status');
  if (ctxStatus) {
    new Chart(ctxStatus, {
      type: 'pie',
      data: {
  ...
  const ctxContributions = document.getElementById('stats-contributions');
  if (ctxContributions) {
